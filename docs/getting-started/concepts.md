...
PointlesSQL doc.  Sprint 22.2 expands each section into a
...
agent_runs:           one row per agent session
  └─ agent_run_operations:  one row per pql.* call
       ├─ training_params_json (Phase 21.3, for train_model ops)
       └─ env_snapshot          (Phase 21.4, hardware fingerprint)
```
