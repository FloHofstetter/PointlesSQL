# Time-travel value-query walkthrough (Sprint 20.3)

Exercises the Phase 20 time-travel surface: a table-detail
...
``agent_run_operations.delta_version_after`` capture from Phase 14.
...
- The metadata DB is on `n4i5j6k7l8m9` (Sprint 20.1) or later —
...
2. **Switch to a historical version.**
   ...
   (Sprint 14 attribution).  Pick `v0`.  The preview pane
   re-fetches via
...
- `query_history` gains one row per `preview-at-version` /
  ...
  the Phase 14.2 read-audit surfaces ordinary `pql.table()`
  calls.
...
- No write paths are added.  Time-travel writes belong to the
  Phase-16 rollback flow that already shipped.
