...
Sprint 35's mobile pass — exercised at three viewport sizes
...
4. **List tables → label/value cards**.
   ...
   - Repeat on the Sprint-34 Columns card of
...
1. **No mobile degradation**.
   - Action: `browser_resize(1280, 800)` + spot-check `/catalogs/demo`,
     ...
     `/notebook`, `/dashboards`.
   - Assert: every page matches its pre-Sprint-35 appearance —
...
## Found bugs (Sprint 35)

_No bugs surfaced on the Sprint-35 replay (2026-04-17, commit
...
- _At **375 px**: the top navbar collapses to hamburger + brand +
  ...
  `attr(data-label)`). The Sprint-33 Sort-by dropdown appears
...
- _At **1280 px**: no mobile CSS leaks — `/jobs` renders with
  the full Sprint-33 row layout + sortable headers; the sidebar
