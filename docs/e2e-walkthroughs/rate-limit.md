...
Exercises the Sprint 43 per-IP and per-email rate limiter on the
...
`rate_limit.blocked` row to `audit_log` so the Sprint-41
...
6. **`/api/*` stays reachable (Sprint 43 deliberately excludes it)**.
   - Action:
     ```bash
   ...
   - Assert: response is `401` (from `auth_middleware`), NOT `429`.
     A Phase-12 sprint layers limits on `/api/sql/*` after the SQL
     editor lands.
...
_No bugs surfaced. Sprint 43 replay confirmed (Firefox via Playwright
...
- _`POST /api/jobs` with no credentials returned 401 from
  ...
  surface is intentionally out of Sprint 43 scope and the limiter
...
- _`/admin/audit?action=rate_limit.blocked` rendered the two
  ...
  and the `user` column as `anon` — the Sprint-41 viewer surfaces
