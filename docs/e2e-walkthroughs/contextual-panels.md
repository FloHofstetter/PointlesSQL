...
Exercises the Phase 24 context-panels: the 240-px sidebar to the
...
- Why it matters: this is the original Phase-17 panel and the
  reference pattern every Phase-24 panel mirrors.
...
| **L6** | `uv run pytest tests/test_help_registry.py -q` passes — every Phase-24 slug resolves and obeys the length caps |

## Bugs and fixes

None known on Phase-24 land.  Pre-existing bug in
