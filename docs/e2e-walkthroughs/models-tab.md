...
Exercises the Phase 21.5 registered-models browse surface
...
- A test catalog `pql_test` exists with a writeable schema
  `mlflow_smoke`.  The Sprint 21.1 smoke test
...
- Tag editing on registered_model securables (Soyuz Sprint 25
  excludes `registered_model` from `TagSecurableType`).
- Champion/challenger promotion UI (Sprint 21.6).
- Inference lineage from a model to downstream prediction tables
  (Sprint 21.7).
- Forced autolog on PQL training hooks (Sprint 21.3).
