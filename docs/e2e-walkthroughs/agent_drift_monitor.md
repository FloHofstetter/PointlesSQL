...
**Sprint 13.5** — replay this playbook to verify the Drift-Monitor
...
shipped Phase-13 primitives in one flow:

- **Sprint 13.2** registry routes (`POST /api/agent-runs`, `/finish`)
- **Sprint 13.3** CloudEvents envelope (`pointlessql.agent_run.*`)
- **Sprint 13.4** control-room `/runs` filter bar + detail card deck
- **Sprint 13.6** `X-Principal` header forwarded into the PQL
  session
- **Sprint 13.5.4** Medallion conformance check on `/runs/{id}`

This is **not** the Hermes-Medallion demo (Sprint 13.5.5 — that one
...
- **Run metadata** card lists Principal `drift-monitor@ops.local`,
  ...
  badges.
- **Medallion conformance** card (Sprint 13.5.4) — if the bronze
...
Sprint 13.6's principal forwarding means the audit log shows
