...
1. **Open a fresh notebook** — navigate to
   ...
   — **no** ``pql_cell_id="…"`` segment (Sprint 96 invariant).
...
5. **D.04 — markdown cell** — set the source to a
   ...
   - Assert: the marker renders as ``# %% [markdown]`` (no UUID).
     ...
     is hidden from the Monaco view (pre-Sprint-98 behaviour).
     Screenshot: ``05-markdown-cell.png``.
     - **Known limitation** (tracked as BUG-98-01 — superseded
       by the Sprint 97 dirty-flag + Sprint 98 zone-prune work,
...
6. **D.04b — ``display(Markdown("…"))``** (Sprint 98 BUG-98-02
   ...
   - Assert: the output zone shows the **rendered** markdown —
     ...
     pre-Sprint-98 ``<IPython.core.display.Markdown object>``
...
12. **External edit reload** — from another terminal,
    ...
    - Assert: the edit shows up; no 500 from the parser.  The
      Sprint 97 tolerance guards keep the editor open.

13. **Markerless file tolerance** (Sprint 97) — create a
...
14. **BOM / CRLF tolerance** (Sprint 97) — from a shell, write a
...
- **BUG-98-02** — ``display(Markdown("…"))`` rendered the
  repr.  **Fixed** in Sprint 98 by adding a ``text/markdown``
...
- **BUG-98-05** — output zones keyed by the transient
  ...
  in Sprint 98 by adding ``pruneOrphanOutputZones(alive)``
