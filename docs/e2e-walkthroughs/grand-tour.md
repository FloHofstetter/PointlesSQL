...
- One-shot seed:
  ...
  the Phase-2 ephemeral state (dashboards, alerts, saved queries,
...
### Act 3 — Phase-15.8 lineage spotlight (4 steps, ~5 min)

This act exercises the row / column / value lineage that Phase 15.8
...
"Phase 15.8 lineage acceptance" block in
...
1. **Row-trace from a silver row**.
   ...
   - Assert: URL pattern
     ...
     `_lineage_row_id` per Phase-15.8.2's `h._lineage_row_id`
...
3. **Operations tab — version anchors**.
   ...
   - Assert: rows show the `write_table` and `merge` operations with
     ...
     (Phase-15 audit anchor — these are NEVER NULL after a
...
4. **Lineage tab — Graph sub-tab**.
   ...
   - Assert: cytoscape DAG renders with the silver node centred;
     ...
     + gold.test_set downstream (Phase 17.3.1 lazy-load — the canvas
...
5. **Audit tab — External writes**.
   ...
   - Assert: the sub-tab renders even with zero external writes; the
     ...
     not a 404 (Phase 14 wiring sanity check).
...
This act is the Phase-21.7 + Phase-15.8.3 confluence —
...
3. **Lineage tab — bidirectional DAG**.
   ...
   - Assert: cytoscape DAG paints THREE node types:
     ...
     - The downstream edge requires `source_model_uri` on the
       prediction's row-edges — that's the Phase-15.8.3 +
       Phase-21.7 combined contract.
...
5. **Promotion tab**.
   ...
   - Assert: "Current champion" card shows v1 with the Champion
     ...
     populating it is a Phase-21.6 follow-up that depends on
...
3. **Alerts index**.
   ...
   - Assert: at least one entry — "Phase-2 demo alert" wired to the
     "Phase-2 demo saved query" with `cron_expr=0 0 * * *`.
...
2. **Saved audit queries**.
   ...
   - Assert: list of audit queries including
     "Phase-2 audit query" (preserved by `--keep-state`) PLUS the
...
- **Seed-demo Phase-2 coverage script itself** — see
  [`full-stack-demo.md`](full-stack-demo.md) for the 25 steps and
  the post-replay acceptance queries (including the Phase-15.8
...
| `BUG-grand-02` | doc | Act 3 step 1 | Row-trace shows `bronze.houses_raw` only as upstream (1 hop, fan-in × 14), not "houses + sales". Correct by design — silver row-id derives from the houses-side via `h._lineage_row_id` projection (Phase 15.8.2). | ✅ doc fixed 2026-05-01 — Act 3 step 1 says "≥ 1 hop to bronze.houses_raw" with the Phase-15.8.2 explanation. |
...
All five-Phase-15.8-relevant assertions PASSED on first replay:
...
- `/dashboards` shows the preserved `Phase-2 demo dashboard` ✓
