...
**Sprint 13.5.5** — closes Phase 13 + 13.5.  Replay this
playbook to verify a real Hermes agent (running in its own
sandbox) authors a three-layer Medallion lakehouse from a CSV
in a UC Volume, with the full Sprint-13.8 forced audit trail
visible in `/runs/{id}`.

This is the **Phase 13 done moment**: the first reproducible
end-to-end flow where an agent — not a human — writes the
bronze / silver / gold transformations.

The walkthrough exercises every primitive Phase 13 + 13.5 + 13.7
shipped in one run:
...
1. **PointlesSQL** running on `http://127.0.0.1:8000` with the
   Phase-13.x routes (`/api/agent-runs`, `/api/sql/*`,
...
`hermes_medallion.py` bytes as `source` (Sprint 13.8).
...
4. **Queries** — every SQL the LLM (or the `pql.sql` primitive)
   ...
   to this run via the Sprint-13.9 `agent_run_id` column.
...
The Sprint-54 stats should appear on each layer's catalog page:
...
(Sprint-13.5.3 SHA-based dedup on autoload, Sprint-13.5.2 upsert
...
- **Tier-3 audit gaps** stay open (soyuz UC mutation
  ...
  See `project_phase13_audit_gaps.md`; Phase-15+ shoreguard
