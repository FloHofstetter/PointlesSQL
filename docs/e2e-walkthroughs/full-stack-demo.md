...
run it walks the entire stack — every PQL primitive, every Phase-21
...
inference → 24-route smoke tour); steps 13-22 are the **Phase-2
...
### Phase-2 coverage extension (steps 13-22, default)
...
23. **Read-tour (extended)** — the original 24 routes plus every
    other Phase-2 read surface: deep catalog walk, tree-search,
...
# Default — full Phase-2 coverage (~30 s, 155 routes).  ``--login-email``
...
and exits in ~30 s.  The Phase-2 stateful steps are idempotent (every
CRUD step deletes its own state), but a re-run without `--fresh` is
NOT supported because Phase-1 promoted-branch state on `demo_ml.gold`
...
state lights up — catalog, lineage (with the Phase-15.8 spotlight),
...
ephemeral Phase-2 objects (dashboards, alerts, saved queries,
...
Phase 15.8 closes the spec/end-to-end-loop gap surfaced by the
first Phase-2 replay (where the demo silently produced 0 rows on
...
Phase 21 closure left "manual e2e replay" as a user-job — and for
