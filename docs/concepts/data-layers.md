...
The autoload primitive (`pql.autoload`, Sprint 13.5.3) injects
these columns automatically; manual writes are checked passively
by the conformance check on `/runs/{id}` (Sprint 13.5.4).
...
[`pointlessql.yaml.example`](../pointlessql.yaml.example) to
