...
| `source_model_uri` | str? | inference attribution (Phase 21.7) |

Every row carries an `_lineage_row_id` synthetic column at the
storage level (Sprint 15) — a deterministic hash that survives
re-writes.
...
`hash_only` (Phase 20.2).

## Inference-level (Phase 21.7)
...
primitive (Phase 15.5) was added specifically because pure
...
The Run-detail page's **Rejects tab** (Phase 15.5) renders this.
Auditors can answer "why didn't this customer end up in the
dim table?" without re-running the merge.

## Limitations

- **External writes are detected, not parsed.**  Phase 14.3
