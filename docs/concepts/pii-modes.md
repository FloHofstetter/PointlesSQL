...
the Phase-18 render-time masking only; that masking still gates
...
pre-Sprint-20.1 data see a soft transition: historical clear values
stay readable to admins via the existing render-time masking; new
value-changes hash.

To opt-out (keep pre-Phase-20 behaviour): set
