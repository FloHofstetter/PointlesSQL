...
| **Query** | One DuckDB SQL execution | `query_history` | Read paths; tied to the run via `agent_run_id` (Sprint 13.9) |
...
| `op_name` | str | `write_table`, `merge`, `aggregate`, `rollback`, `branch_promote`, `train_model` (Phase 21.3) |
...
| `mlflow_run_id` | str? | cross-link to MLflow when the op is `train_model` (Phase 21.2) |
| `training_params_json` | text? | hyperparameters + metrics blob (Phase 21.3) |
| `env_snapshot` | text? | hardware + library fingerprint (Phase 21.4) |
...
// op_name = "train_model" (Phase 21.3)
{
  "framework": "sklearn",
...
These four additions turn the data-engineering audit (Phase 14)
...
loop.  Phase 16's `pql.rollback` reads the op-row and asks
...
## Read paths: query_history (Sprint 13.9)

The op-row covers writes.  For reads, Sprint 13.9 added
