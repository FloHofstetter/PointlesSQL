...
fan-out shape.  Phase 22+ candidates (e.g. `dataset_certification`,
...
(Phase 16) and `pointlessql.model.promoted` (Phase 21.6) — one
shared envelope for every interesting state change in the
system.

## The four canonical bot personas

Phase 19 designed four reference Hermes personas that exercise
