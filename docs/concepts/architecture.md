...
graph TB
    ...
    subgraph "PointlesSQL process (Python)"
        ...
        ML["MLflow subprocess<br/>(Phase 21)"]
    end
...
`mlruns/` next to the warehouse.  See [Phase 21
...
│   ├── pql_training_routes.py     # /api/pql/training/log (Phase 21.8)
│   ├── models_routes.py           # /models/*  (Phase 21.5+)
│   ├── lineage_routes.py          # /api/lineage/*
│   ├── audit_routes.py            # /api/audit/<axis> (Phase 19.1)
...
└── settings.py                    # pydantic-settings, sub-models per Sprint 45
```
