...
PointlesSQL exposes ~500 HTTP routes across 51 router files.
This page hand-curates the **top-30** routes that an integrator
will actually call — auth, runs, models, lineage, write, merge,
branching, supervisor, auditor. The remaining ~470 routes are
mostly internal HTML rendering for the web UI.

> For the complete REST surface auto-generated from the running
> FastAPI app's OpenAPI schema, see [Full OpenAPI
> reference](api/openapi.md). That page is regenerated by a
> pre-commit hook on every change under `pointlessql/api/`, so
> it cannot drift from the actual route surface.
