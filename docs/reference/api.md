...
> **Sprint 22.5** will add an auto-generated appendix covering
...
| `GET` | `/api/runs/{run_id}/graph` | 🍪 | Cytoscape-shaped DAG of the run's operations + lineage edges (Sprint 17.4). |
...
| `POST` | `/api/pql/training/log` | 🍪 / 🔑 | Phase 21.8 training-log.  Body `{framework, params, metrics, mlflow_run_id?, op_name?, started_at?, finished_at?}`. |
...
| `GET` | `/api/models/{full_name}/predictions` | 🍪 | Tables fed by this model with row counts (Phase 21.7). |
...
| `POST` | `/api/models/{full_name}/promote` | 👮 supervisor | Phase 21.6 champion-flip.  Body `{target_version, reason}`.  Persists `agent_review` + emits `pointlessql.model.promoted`. |
...
| `GET` | `/api/lineage/column-trace?table=...&column=...` | 🍪 | Column-level provenance (Phase 15.6). |
| `GET` | `/api/lineage/value-changes?table=...&row_id=...` | 🕵 auditor | Value-level diffs (Phase 15.7).  Auditor-only because the diffs may carry PII. |
