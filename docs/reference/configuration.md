...
as a pydantic-settings `BaseSettings` sub-model. split
...
| `POINTLESSQL_JUPYTER_PORT` | `8888` | Reserved for future re-introduction; currently unused ( retired the iframe). |
...
| `POINTLESSQL_SQL_COST_GATE_THRESHOLD_ROWS` | `1000000` | : queries with EXPLAIN row estimate ≥ this need confirmation. |
...
. Reads `POINTLESSQL_AUDIT_STREAM_*`.
...
## External-write detection
 ...
 scanner. Reads `POINTLESSQL_EXTERNAL_WRITES_*`.
...
| `POINTLESSQL_MLFLOW_REGISTRY_URI` | derived → `uc:{soyuz_url}` | MLflow UC-OSS registry URI ('s `uc:` scheme, not `uc-oss:`). |
