...
async function _loadCodeMirrorModules() {
  ...
  const [stateMod, viewMod, commandsMod, languageMod, sqlMod, pythonMod, markdownMod, themeMod] =
    await Promise.all([
...
export function cellEditor({
  ...
  return {
    ...
    async mount(host) {
      ...
      const yBindingActive = !!(this._yBinding && this._yBinding.ytext);
      ...
      if (yBindingActive) {
        ...
        this._ytextObserver = () => {
          try {
            this._onSourceChange(ytext.toString());
          } catch (_e) {
            /* swallow */
          }
        };
      ...
      this._cmView = new EditorView({
        state: EditorState.create({
          doc: yBindingActive ? this._yBinding.ytext.toString() : this._initialSource,
          extensions,
        }),
        parent: host,
      });
      ...
      if (yBindingActive) {
        ...
        if (ytext.length === 0 && this._initialSource) {
          try {
            ytext.insert(0, this._initialSource);
          } catch (_e) {
            /* swallow */
          }
        }
    ...
    destroy() {
      if (this._ytextObserver && this._yBinding && this._yBinding.ytext) {
        try {
          this._yBinding.ytext.unobserve(this._ytextObserver);
        } catch (_e) {
          /* swallow */
        }
        this._ytextObserver = null;
