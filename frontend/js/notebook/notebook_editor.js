  ...
  const state = {
      ...
      if (kind === 'kernel') {
        switch (this.kernelStatus) {
          case 'ready':
            return 'bi bi-cpu';
          case 'connecting':
            return 'bi bi-arrow-repeat pql-spin';
          case 'closed':
          case 'disconnected':
          default:
    ...
    async init() {
      try {
        ...
        if (!res.ok) {
          this.errorMessage =
            (res.data && res.data.detail) || `Failed to load notebook (HTTP ${res.status}).`;
          this.loading = false;
          return;
        }
    ...
    _setupScrollPastEnd() {
      ...
      const recompute = () => {
        ...
        const cellTopInScrollFrame = lastRect.top - contentRect.top + content.scrollTop;
        ...
        const desiredScrollHeight = cellTopInScrollFrame + visible - TARGET_TOP_OFFSET;
        const currentScrollHeight = content.scrollHeight;
        const currentPadding = parseFloat(getComputedStyle(shell).paddingBottom) || 0;
