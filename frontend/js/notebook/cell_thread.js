...
export function cellThread({ notebookUuid, cell, initialCounts, curatedTags = [] } = {}) {
  return {
    ...
    get hasActivity() {
      return (
        this.commentCount > 0 || this.followerCount > 0 || this.reactions.some((r) => r.count > 0)
      );
    },
    ...
    get approvedReviewCount() {
      return this.reviewComments.filter((c) => this.decisionFor(c).id === 'approved').length;
    },

    get changesRequestedCount() {
      return this.reviewComments.filter((c) => this.decisionFor(c).id === 'changes_requested')
        .length;
    },
    ...
    async loadExplanations() {
      ...
      if (!cellUuid) return;
      try {
        const resp = await jsonFetch(`/api/notebook/chat/cell/${cellUuid}/explanations`);
    ...
    async submitReview() {
      ...
      const decision =
        REVIEW_DECISIONS.find((d) => d.id === this.reviewDecision) || REVIEW_DECISIONS[0];
    ...
    async toggleReaction(emoji) {
      ...
      try {
        if (current.mine) {
          await jsonFetch(`${this.baseUrl}/reactions/${encodeURIComponent(emoji)}`, {
            method: 'DELETE',
          });
          current.mine = false;
