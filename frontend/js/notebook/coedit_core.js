...
export function installCoeditCore(state, { userInfo = null } = {}) {
  ...
  state._initCoedit = function () {
    ...
    this._coedit = createCoeditClient({
      ...
      onStatusChange: (next) => {
        this.coeditStatus = next;
      },
      onCellRemap: (remap) => {
        this._applyCellUuidRemap(remap);
      },
      onAgentPresence: (presence) => {
        this._applyAgentPresence(presence);
      },
      onSynced: () => {
  ...
  state.coeditPillClass = function () {
    switch (this.coeditStatus) {
      case 'live':
        return 'pql-vital-pill pql-vital-pill--success';
      case 'connecting':
      case 'offline':
        return 'pql-vital-pill pql-vital-pill--warning';
      case 'error':
        return 'pql-vital-pill pql-vital-pill--danger';
      case 'unauthorized':
      default:
  ...
  state.coeditTooltip = function () {
    switch (this.coeditStatus) {
      case 'live':
        return 'Co-edit channel connected — changes sync across open tabs.';
      case 'connecting':
        return 'Opening co-edit channel…';
      case 'offline':
        return 'Co-edit channel dropped — reconnecting.';
      case 'unauthorized':
        return 'You are viewing this notebook in read-only mode.';
      case 'error':
        return 'Co-edit channel unavailable on this host.';
      default:
        return 'Co-edit channel idle.';
    }
  };
}
