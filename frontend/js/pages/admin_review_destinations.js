    ...
    return {
        ...
        async remove() {
            const ok = window.pqlConfirm
                ...
                : window.confirm('Delete destination "' + this.name + '"? Historical fan-out lines survive.');
            if (!ok) return;
            this.busy = true;
            const res = await window.pqlApi.fetch(
                '/api/admin/review-destinations/' + this.destId,
