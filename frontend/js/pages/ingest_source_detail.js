...
export function ingestSourceDetail(sourceId) {
    return {
        ...
        async loadRuns() {
            ...
            const res = await window.pqlApi.fetch(
                '/api/ingest/sources/' + this.sourceId + '/runs'
            );
        ...
        async saveSchedule() {
            ...
            const res = await window.pqlApi.fetch(
                '/api/ingest/sources/' + this.sourceId + '/schedule',
                {
                    method: 'PUT',
        ...
        async clearSchedule() {
            ...
            const res = await window.pqlApi.fetch(
                '/api/ingest/sources/' + this.sourceId + '/schedule',
                { method: 'PUT', body: { cron_expr: null } }
            );
            this.savingSchedule = false;
            if (!res.ok) {
        ...
        async pullNow() {
            this.pulling = true;
            const res = await window.pqlApi.fetch(
                '/api/ingest/sources/' + this.sourceId + '/pulls',
                { method: 'POST', body: {} }
            );
            this.pulling = false;
            if (!res.ok) {
            ...
            window.pqlToast?.success?.(
                `Pulled ${ok} mapping(s)` + (fail ? `, ${fail} failed.` : '.')
            );
            await this.load();
            await this.loadRuns();
        }
