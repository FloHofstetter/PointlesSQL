  ...
  return {
    ...
    async init() {
      ...
      try {
        ...
        /* runs unavailable — silent no-op */
      }
      this.dbtRunsLoading = false;

      try {
        const url =
          `/api/ml/table-relations?catalog=${encodeURIComponent(catalog)}` +
      ...
      try {
        const url =
          `/api/models?catalog_name=${encodeURIComponent(catalog)}` +
        ...
        /* MLflow not configured — silent no-op */
      }
      this.mlModelsLoading = false;
      this.mlAvailable = this.mlModels.length > 0 || Object.keys(this.mlModelByTable).length > 0;
    },
  };
}
