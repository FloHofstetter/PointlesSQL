...
function _init() {
    ...
    async function loadInbox() {
        ...
        rowsEl.innerHTML = '<tr><td colspan="9" class="py-3"><div class="placeholder-glow" aria-hidden="true"><div class="placeholder col-7 mb-2"></div><div class="placeholder col-9 mb-2"></div><div class="placeholder col-5"></div></div></td></tr>';
        ...
        if (!data.anomalies.length) {
            rowsEl.innerHTML = '<tr><td colspan="9" class="text-center text-muted py-4">No active anomalies for the chosen filters.</td></tr>';
            return;
        }
    ...
    selectAllEl.addEventListener('change', () => {
        ...
        if (selectAllEl.checked) {
            ackable.forEach((a) => selected.set(rowKey(a), {
                metric: a.metric, bin_iso: a.bin_iso, bin_kind: a.bin_kind,
            }));
        } else {
            selected.clear();
        }
    ...
    bulkAckBtn.addEventListener('click', async () => {
        ...
        const comment = prompt('Optional ack comment for all ' + selected.size + ' (leave blank for none)');
