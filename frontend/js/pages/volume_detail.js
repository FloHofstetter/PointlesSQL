...
export function volumeDetail({ fullName, files } = {}) {
  return {
    ...
    async submitUpload() {
      ...
      try {
        const res = await fetch('/api/volumes/' + encodeURIComponent(this.fullName) + '/files', {
          method: 'POST',
    ...
    async removeFile(file) {
      ...
      const res = await window.pqlApi.fetch(
        '/api/volumes/' + encodeURIComponent(this.fullName) + '/files/' + file.path,
    ...
    async convertToDelta(file) {
      ...
      const table = window.prompt(
        'Target table name in ' + this.fullName.split('.').slice(0, 2).join('.') + ':',
        suggested
      );
      if (!table) return;
      const res = await window.pqlApi.fetch(
        '/api/volumes/' + encodeURIComponent(this.fullName) + '/convert-to-delta',
        {
          method: 'POST',
      ...
      if (res.ok && res.data) {
        if (window.pqlToast) {
          window.pqlToast.success('Registered as Delta table: ' + res.data.full_name);
        }
