...
export function pathFromUrl() {
  ...
  const mdl = window.location.pathname.match(/^\/models\/([^/.]+)\.([^/.]+)\.([^/.]+)/);
  if (mdl) {
    return {
      catalog: mdl[1] || '',
      schema: mdl[2] || '',
  ...
  const vol = window.location.pathname.match(/^\/volumes\/([^/.]+)\.([^/.]+)\.([^/.]+)/);
  if (vol) {
    return {
      catalog: vol[1] || '',
      schema: vol[2] || '',
...
export function catalogTree() {
  return {
    ...
    starredHref(s) {
      ...
      if (s.kind === 'table')
        return '/catalogs/' + s.catalog + '/schemas/' + s.schema + '/tables/' + s.table;
      return '#';
    },
    starredIcon(s) {
      if (!s) return 'bi-star';
    ...
    async fetchServerSearch(q) {
      ...
      try {
        ...
        const data = await res.json();
        this.searchResults = Array.isArray(data.matches) ? data.matches : [];
