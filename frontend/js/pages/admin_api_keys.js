  ...
  return {
    ...
    async revoke() {
      if (
        !window.confirm(
          'Revoke API key "' +
            this.name +
            '"? Bearer requests using this secret will be rejected immediately.'
        )
      ) {
        return;
      }
    ...
    async rotate() {
      if (
        !window.confirm(
          'Rotate API key "' +
            this.name +
            '"? A successor will be minted; the current key stays valid for 24h so clients can pick up the new secret.'
        )
      ) {
        return;
      }
    ...
    async quarantine() {
      const reason = window.prompt(
        'Reason for quarantining "' + this.name + '"? (≤200 chars; required for audit trail)'
      );
    ...
    async unquarantine() {
      if (
        !window.confirm(
          'Lift quarantine on "' + this.name + '"? Bearer auth will resume immediately.'
        )
      ) {
        return;
      }
