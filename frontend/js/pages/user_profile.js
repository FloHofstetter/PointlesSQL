...
export function userProfile(target, ctx) {
    ...
    return {
        ...
        loaded: false,
        profile: { user_id: target.user_id, email: target.email, display_name: target.display_name, bio_md: '', location: null },
        counts: {},
        stewarded: [],
        badges: [],
        recentComments: [],
        ...
        async follow() {
            try {
                const res = await fetch('/api/users/' + this.target.user_id + '/follow', { method: 'POST' });
                if (!res.ok) throw new Error('HTTP ' + res.status);
        ...
        async unfollow() {
            try {
                const res = await fetch('/api/users/' + this.target.user_id + '/follow', { method: 'DELETE' });
                if (!res.ok) throw new Error('HTTP ' + res.status);
