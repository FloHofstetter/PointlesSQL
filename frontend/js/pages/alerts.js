...
export function alertsPage({ alerts, savedQueries } = {}) {
  return {
    ...
    init() {
      /* data seeded server-side via x-data */
    },
    ...
    async rotateToken() {
      ...
      const res = await window.pqlApi.fetch('/api/me/feed-token/rotate', {
        method: 'POST',
        silent: true,
      });
      if (res.ok && res.data) {
    ...
    async toggleActive(alert) {
      const res = await window.pqlApi.fetch('/api/alerts/' + encodeURIComponent(alert.slug), {
        method: 'PATCH',
    ...
    async bulkDelete() {
      ...
      await Promise.all(
        slugs.map(async (slug) => {
          const res = await window.pqlApi.fetch('/api/alerts/' + encodeURIComponent(slug), {
            method: 'DELETE',
            silent: true,
          });
      ...
      if (window.pqlToast) {
        if (failed === 0) {
          window.pqlToast.success(
            'Deleted ' + slugs.length + ' alert' + (slugs.length === 1 ? '' : 's') + '.'
          );
        } else {
    ...
    async bulkToggleActive() {
      ...
      let failed = 0;
      await Promise.all(
        selectedAlerts.map(async (a) => {
          if (a.is_active === targetActive) return;
          const res = await window.pqlApi.fetch('/api/alerts/' + encodeURIComponent(a.slug), {
            method: 'PATCH',
      ...
      if (window.pqlToast) {
        if (failed === 0) {
          window.pqlToast.success(
            (targetActive ? 'Resumed ' : 'Paused ') + selectedAlerts.length + ' alert(s).'
          );
        } else {
