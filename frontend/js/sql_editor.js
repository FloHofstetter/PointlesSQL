...
export function sqlEditor() {
 ...
 const rowsScanned = (typeof node.operator_rows_scanned === 'number')
 ? node.operator_rows_scanned : null;
 const cumCard = (typeof node.cumulative_cardinality === 'number')
 ? node.cumulative_cardinality : null;
 const cumScan = (typeof node.cumulative_rows_scanned === 'number')
 ? node.cumulative_rows_scanned : null;
 const resultSize = (typeof node.result_set_size === 'number')
 ? node.result_set_size : null;
 const peakBuf = (typeof node.system_peak_buffer_memory === 'number')
 ? node.system_peak_buffer_memory : null;
 const peakTmp = (typeof node.system_peak_temp_dir_size === 'number')
 ? node.system_peak_temp_dir_size : null;
 ...
 ? this.formatBytes(v)
 : unit === 'time' ? this.formatTiming(v) : v.toLocaleString();
