...
export const executeMethods = {
  ...
  async run(opts) {
    ...
    if (res.ok && res.data) {
      if (res.data.is_explain) {
        ...
        const hasX = this.chartConfig.x && cols.some((c) => c.name === this.chartConfig.x);
        const hasY = this.chartConfig.y && cols.some((c) => c.name === this.chartConfig.y);
        if (!hasX || !hasY) {
  ...
  async cancel() {
    ...
    const res = await window.pqlApi.fetch(`/api/sql/execute/${encodeURIComponent(qid)}/cancel`, {
      method: 'POST',
      silent: true,
    });
