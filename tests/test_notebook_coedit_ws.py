...
def fresh_notebook() -> Iterator[str]:
    ...
    with factory() as session:
        ...
        session.execute(
            delete(NotebookPermission).where(NotebookPermission.notebook_id == notebook_id)
        )
...
def test_ws_anonymous_closes_with_4401() -> None:
    ...
    with TestClient(app) as client:
        with pytest.raises(WebSocketDisconnect) as exc_info:
            with client.websocket_connect(f"/ws/notebook/coedit/{notebook_id}") as ws:
                ws.receive_bytes()
...
def test_ws_unknown_notebook_closes_with_4404(
    ...
    with TestClient(app, cookies=auth_cookies) as client:
        with pytest.raises(WebSocketDisconnect) as exc_info:
            with client.websocket_connect(f"/ws/notebook/coedit/{bogus_id}") as ws:
                ws.receive_bytes()
...
def test_ws_viewer_role_closes_with_4403(
    ...
    with TestClient(app, cookies=non_admin_cookies) as client:
        with pytest.raises(WebSocketDisconnect) as exc_info:
            with client.websocket_connect(f"/ws/notebook/coedit/{fresh_notebook}") as ws:
                ws.receive_bytes()
...
def test_ws_sync_update_broadcasts_to_other_clients(
    ...
    with TestClient(app, cookies=auth_cookies) as client:
        with client.websocket_connect(f"/ws/notebook/coedit/{fresh_notebook}") as ws_a:
            initial_a = ws_a.receive_bytes()
            with client.websocket_connect(f"/ws/notebook/coedit/{fresh_notebook}") as ws_b:
...
def test_ws_awareness_update_relays_but_doesnt_persist(
    ...
    with factory() as session:
        row = session.execute(
            select(NotebookCrdtState).where(NotebookCrdtState.notebook_id == fresh_notebook)
        ).scalar_one_or_none()
    if row is not None:
