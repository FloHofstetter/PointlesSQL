...
def test_parse_keys_supports_newline_and_comma() -> None:
    ...
    assert parsed == {
        "hermes": ("abc123", False, False, False, False, False),
        "paperclip": ("xyz789", False, False, False, False, False),
        "bot": ("secret", False, False, False, False, False),
    }
...
def test_parse_keys_supports_supervisor_scope() -> None:
    ...
    parsed = api_keys_service.parse_keys(raw)
    assert parsed == {
        "alice": ("s1", False, False, False, False, False),
        "sup": ("godkey", True, False, False, False, False),
    }
...
def test_parse_keys_supports_auditor_scope() -> None:
    ...
    assert parsed == {
        "alice": ("s1", False, False, False, False, False),
        "aud": ("k", False, True, False, False, False),
    }


def test_parse_keys_supports_lineage_inbound_scope() -> None:
    raw = "alice:s1, fed:k:lineage_inbound"
    parsed = api_keys_service.parse_keys(raw)
    assert parsed == {
        "alice": ("s1", False, False, False, False, False),
        "fed": ("k", False, False, True, False, False),
    }
...
def test_parse_keys_supports_analyst_scope() -> None:
    ...
    assert parsed == {
        "alice": ("s1", False, False, False, False, False),
        "lens": ("k", False, False, False, True, False),
    }
