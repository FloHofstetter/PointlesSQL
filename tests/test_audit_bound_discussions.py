"""Tests for the Phase-72.5 audit-bound discussion mirror.
...
def _audit_rows(action: str) -> list[AuditLog]:
    ...
    with factory() as session:
        return (
            session.execute(select(AuditLog).where(AuditLog.action == action))
            .scalars()
            .all()
        )
...
@pytest.mark.asyncio
async def test_post_drops_audit_log_row(
    ...
    row = rows[0]
    assert row.target == (
        f"data_product:main.sales_gold#tab-discussion-comment-{comment_id}"
    )
...
@pytest.mark.asyncio
async def test_long_body_preview_truncated(
    tmp_path: Path, admin_client: httpx.AsyncClient
) -> None:
...
@pytest.mark.asyncio
async def test_delete_drops_audit_log_row(
    tmp_path: Path, admin_client: httpx.AsyncClient
) -> None:
    ...
    assert rows[0].target == (
        f"data_product:main.sales_gold#tab-discussion-comment-{cid}"
    )
...
@pytest.mark.asyncio
async def test_audit_row_carries_workspace(
    tmp_path: Path, admin_client: httpx.AsyncClient
) -> None:
