...
polymorphic follow table lands in Phase 77.8.
"""
...
async def follow_social_entity(
    ...
    return await follow_polymorphic_entity(
        kind, polymorphic_ref, request
    )
...
async def unfollow_social_entity(
    ...
    polymorphic_ref = parse_ref(kind, ref)
    return await unfollow_polymorphic_entity(
        kind, polymorphic_ref, request
    )
...
async def get_social_followers_count(
    ...
    polymorphic_ref = parse_ref(kind, ref)
    return await get_polymorphic_followers_count(
        kind, polymorphic_ref, request
    )
...
async def list_social_followers(
    ...
    polymorphic_ref = parse_ref(kind, ref)
    return await list_polymorphic_followers(
        kind, polymorphic_ref, request
    )
