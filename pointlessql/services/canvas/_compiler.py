...
def compile_nodes(nodes: list[dict[str, Any]]) -> tuple[str, list[str]]:
    ...
    for idx, k in enumerate(kinds):
        ...
        if idx < len(kinds) - 1 and k in ("write_dp", "run_model"):
            errors.append(
                f"Node {idx}: {k} must be the last node — branching is not supported."
            )
    ...
    for idx, node in enumerate(nodes):
        ...
        if kind == "read_dp":
            ...
            if not _FQN_RE.match(fqn):
                errors.append(
                    f"read_dp node {idx}: table_fqn must be three-part."
                )
                continue
            ...
            if not _FQN_RE.match(right_fqn):
                errors.append(
                    f"join node {idx}: right_table_fqn must be three-part."
                )
                continue
            if not on_col:
            ...
            lines.append(
                f"{current_var} = {current_var}.merge({tmp}, on={on_col!r}, how={how!r})"
            )
        elif kind == "group_by":
            ...
            if not isinstance(cols, list) or not cols:
                errors.append(
                    f"group_by node {idx}: at least one column is required."
                )
                continue
            if not isinstance(aggs, dict) or not aggs:
                errors.append(
                    f"group_by node {idx}: at least one aggregate is required."
                )
                continue
            lines.append(
            ...
            if not model_uri:
                ...
                continue
            lines.append(
                f'predictions = pql.run_model("{model_uri}", {current_var})'
            )
            ...
            if not _FQN_RE.match(target):
                errors.append(
                    f"write_dp node {idx}: target_fqn must be three-part."
                )
                continue
            if mode not in ("append", "overwrite", "merge"):
                errors.append(
                    f"write_dp node {idx}: mode must be append / overwrite / merge."
                )
                continue
            if mode == "merge":
                on_col = str(cfg.get("on") or "")
                if not on_col:
                    errors.append(
                        f"write_dp node {idx}: merge mode requires 'on'."
                    )
                    continue
                lines.append(
                    f'pql.merge("{target}", {current_var}, on={on_col!r})'
                )
            else:
                lines.append(
                    f'pql.write_table("{target}", {current_var}, mode={mode!r})'
                )
