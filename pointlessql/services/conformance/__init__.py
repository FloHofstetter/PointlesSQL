"""Medallion conformance checks — passive surface.
...
about it.  Selected checks could later be converted into shoreguard
...
layer the table is silently passed; an explicit ``layer`` UC tag
(``layer_tag_key`` setting) is the future hook for overriding the
schema-name heuristic, but the soyuz-catalog generated client
doesn't surface tags directly yet — schema-name match is the MVP.
"""
