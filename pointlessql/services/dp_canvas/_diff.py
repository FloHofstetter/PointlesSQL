...
def _node_diff_from(node: CanvasNode, *, before: bool) -> CanvasNodeDiff:
    ...
    if before:
        return CanvasNodeDiff(
            id=node.id, block_type=node.block_type, before_config=cfg
        )
    return CanvasNodeDiff(
        id=node.id, block_type=node.block_type, after_config=cfg
    )
...
def diff_docs(before: CanvasDoc, after: CanvasDoc) -> CanvasDiff:
    ...
    removed_nodes = [
        _node_diff_from(n, before=True)
        for nid, n in before_nodes.items()
    ...
    for nid, b_node in before_nodes.items():
        ...
        if a_node is None:
            continue
        if (
            b_node.block_type != a_node.block_type
            or b_node.config != a_node.config
        ):
    ...
    added_edges = [
        ...
        if k not in before_edge_keys
    ]
    removed_edges = [
        _edge_diff_from(e)
        for k, e in before_edge_keys.items()
